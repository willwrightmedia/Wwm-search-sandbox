...
st.set_page_config(page_title="World Wide Monitor | Executive Sandbox", page_icon="📡", layout="wide")
...
# --- PYDANTIC SCHEMA: EXPANDED WITH MEDIA OUTLET & BYLINE ---
...
class ReleaseFactItem(BaseModel):
    source_title: str
    source_url: str
    media_outlet: str = Field(description="Name of the news publisher or official site (e.g., ABC News, RMIT Media, The Australian).")
    author_byline: str = Field(description="Journalist or producer name if available. Use 'Official Announcement' or 'Uncredited' if none.")
...
# --- HELPER FUNCTION: MARKDOWN EXPORT GENERATOR ---
...
if st.button("Run Intelligence Search (-50 Tokens)"):
    if not final_query.strip():
        ...
        with st.status("Deploying Gemini Agent Swarm with Native Google Search...", expanded=True) as status:
            st.write("🔍 **Gemini Search Agent:** Searching live media, resolving publisher outlets & journalist bylines...")
            ...
            try:
                ...
                1. Focus search on Australian media, news portals, and official releases (.gov.au, .edu.au).
                2. Extract ONLY factual events, policy commitments, entity statements, and dates.
                3. Explicitly identify the media outlet/publisher name and the journalist/author byline for each piece of coverage.
                4. Disregard PR fluff and promotional narrative.
                """
...
if "current_brief" in st.session_state:
    ...
    for item in brief["items"]:
        with st.expander(f"📌 {item['source_title']}"):
            st.markdown(f"**Media Outlet:** `<span class='media-badge'>{item['media_outlet']}</span>` &nbsp;|&nbsp; **Byline:** *{item['author_byline']}*", unsafe_allow_html=True)
            st.markdown("<br>", unsafe_allow_html=True)
